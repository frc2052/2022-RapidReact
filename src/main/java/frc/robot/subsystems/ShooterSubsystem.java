...
public class ShooterSubsystem extends SubsystemBase {
  ...
  /**
   * Calculates required velocity to send a projectile to a point with given
   * distance and height relative to launcher, as well as the angle to fire from
  ...
  public double calculateReqProjectileVelocity(double distanceMeters) {
    ...
    return Math.sqrt(-9.8 * (Math.pow(distanceMeters, 2))
        / (2 * Math.pow(Math.cos(angleRadians), 2) * (height - distanceMeters * Math.tan(angleRadians))));
  }

  /**
   * Finds angular velocity and converts to RPM by diving by 2PI and multipying by
   * 60.
   * 
   * @param velocity
   * @return
